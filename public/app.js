...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const res = await fetch(`/all?prev=${localStorage.getItem(KEY_PREVIOUS_TIP)}&curr=${localStorage.getItem(KEY_CURRENT_TIP)}`);
...
function refreshGtd(data) {
    ...
    const render = todayBegin => item =>
        `<li id="${item.id}" class="gtdItem${item.todo ? '' : ' inactive'}">${item.task} <time class="secondary">${formatTimestamp(item.id, todayBegin)}</time></li>`;
...
document.getElementById('cleanInactiveGtdItems').addEventListener('click', async () => {
    const data = load(KEY_GTD).filter(e => !e.todo).map(e => {
        e.active = false;
        return e;
    });
...
function renderGuerrilla(data) {
    ...
    document.getElementById('guerrillaGroup').innerHTML = links.map(link => `<a href="${link.link}">${link.text}</a>`).join(' | ');
}
